...
import { javaAdvancedCourseCurriculum, Concept, Chapter, Part } from "@/lib/java-advanced-course-data"
import LessonDialog from "@/components/dsa/LessonDialog"
import { cn } from "@/lib/utils"

export default function JavaAdvancedCourseDocsLayout() {
  ...
  for (const part of javaAdvancedCourseCurriculum) {
    for (const chapter of part.chapters) {
      ...
      if (allLessons.some((l: any) => l.title === activeLesson?.title)) {
  ...
  return (
    <div className="flex h-screen bg-background text-foreground overflow-hidden font-sans">
      ...
      <div className="lg:hidden absolute top-0 left-0 right-0 h-16 border-b border-border bg-card/90 backdrop-blur-xl z-50 flex items-center justify-between px-4">
        <Link href="/programs/java-advanced" className="text-muted-foreground hover:text-foreground transition-colors flex items-center gap-2">
            <ArrowLeft size={16} />
      ...
      <aside className={cn(
        ...
        <div className="hidden lg:flex h-14 border-b border-border/70 dark:border-white/10 items-center justify-between px-4 shrink-0">
          <Link href="/programs/java-advanced" className="group flex items-center gap-2 text-xs font-medium text-muted-foreground hover:text-foreground transition-colors">
            <ArrowLeft size={14} className="transition-transform group-hover:-translate-x-0.5" />
            <span>Advanced Overview</span>
          </Link>
      ...
      <main className="flex-1 relative z-10 flex flex-col bg-background overflow-y-auto custom-scrollbar pt-16 lg:pt-0">
         <header className="hidden lg:flex h-14 border-b border-border/80 dark:border-white/10 items-center justify-between px-8 sticky top-0 bg-background/90 backdrop-blur-md z-30">
            ...
            <div className="flex items-center gap-3 shrink-0">
               <div className="flex items-center gap-2 px-3 py-1 rounded-full border border-primary/30 bg-primary/10">
                 ...
                 <span className="text-xs font-semibold text-primary">18,900 XP</span>
               </div>
               <div className="flex items-center gap-2 px-3 py-1 rounded-full border border-border bg-card">
                 <Zap size={13} className="text-primary" />
                 <span className="text-xs font-semibold text-foreground">6 Day Streak</span>
               </div>
            </div>
         </header>
         ...
         <div className="max-w-4xl mx-auto w-full p-6 lg:p-12 pb-32">
            <AnimatePresence mode="wait">
              <motion.div 
                key={activeLesson?.title || "default"}
              ...
              >
                 ...
                 <div className="space-y-4">
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            ...
                            {activeLesson?.difficulty ? "Challenge" : "Architecture Concept"}
                            </span>
                 ...
                 {(activeLesson?.codeSnippet || activeLesson?.starterCode) && (
                   <div className="mt-10 space-y-3">
                     ...
                     <div className="rounded-2xl border border-border/80 dark:border-white/10 bg-[#181715] dark:bg-[#141413] overflow-hidden text-blue-400 font-mono text-xs shadow-inner">
                       ...
                       <pre className="p-5 text-[13px] font-mono text-slate-200 overflow-x-auto whitespace-pre-wrap leading-relaxed custom-scrollbar">
                         <code>{activeLesson.codeSnippet || activeLesson.starterCode}</code>
                       </pre>
                     </div>
                   </div>
                 )}
                 ...
                 <div className="pt-8 mt-10 border-t border-border flex flex-col sm:flex-row gap-5 items-center justify-between">
                   <div className="flex flex-col text-center sm:text-left">
                     <span className="text-xs font-mono text-muted-foreground uppercase tracking-wider">JVM Benchmark</span>
                     <span className="font-serif text-xl font-normal text-foreground">Interactive Simulation Sandbox</span>
                   </div>
                   <button 
                     onClick={() => setSimulationLesson({ lesson: activeLesson, color: "hsl(var(--primary))" })}
...
function LessonNavItem({ lesson, isActive, onClick }: { lesson: any, isActive: boolean, onClick: () => void }) {
  return (
    ...
    >
      <Coffee size={14} className={cn("mt-0.5 shrink-0", isActive ? "text-primary" : "text-muted-foreground")} />
      <div className="flex flex-col overflow-hidden">
        <span className="leading-snug truncate">{lesson.title}</span>
        {lesson.difficulty && <span className="text-[9px] font-mono uppercase tracking-wider mt-0.5 text-muted-foreground/70">Problem</span>}
      </div>
    </button>
  )
}
